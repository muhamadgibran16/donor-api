...
const getAllStock = async (req, res, next) => {
  try {
    const stock = await db.query("SELECT rs.nama_rs, rs.alamat_rs, tp.id, tp.tipe_darah, rhesus.rhesus, stock.id_rhesus, stock.stock FROM stocks as stock INNER JOIN blood_types AS tp ON stock.id_darah = tp.id INNER JOIN hospitals AS rs ON stock.id_rs = rs.id INNER JOIN rhesus ON stock.id_rhesus = rhesus.id", {
      type: QueryTypes.SELECT
    })
    if (stock.length === 0) {
      return res.status(404).json({
...
const getStockByBloodTypeId = async (req, res, next) => {
  try {
    ...
    const stock = await db.query(`SELECT rs.nama_rs, rs.alamat_rs, tp.id, tp.tipe_darah, rhesus.id, rhesus.rhesus, stock.stock FROM stocks as stock INNER JOIN blood_types AS tp ON stock.id_darah = tp.id INNER JOIN hospitals AS rs ON stock.id_rs = rs.id INNER JOIN rhesus ON stock.id_rhesus = rhesus.id WHERE tp.id = ${id}`, {
      type: QueryTypes.SELECT,
    })
    if (stock.length === 0) {
      return res.status(404).json({
